...
Call log:
  ...
  - waiting for locator('nb-card').filter({ hasText: 'Basic form' }).locator('button')
    9 × locator resolved to <button nbbutton="" tabindex="0" type="submit" status="danger" aria-disabled="false" _ngcontent-rfq-c287="" ng-reflect-status="danger" class="appearance-filled size-medium shape-rectangle status-danger nb-transition">Submit</button>
